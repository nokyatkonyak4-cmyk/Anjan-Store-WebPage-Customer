...
const content = `import { useParams, useNavigate, useSearchParams } from 'react-router-dom';
...
export default function DigitalBillScreen() {
  ...
  if (isLoading) {
      return (
          <div className="flex flex-col min-h-screen max-w-md mx-auto bg-[#F8F9FA] justify-center items-center">
  ...
  if (!order) {
       return (
          <div className="flex flex-col min-h-screen max-w-md mx-auto bg-[#F8F9FA] justify-center items-center">
             ...
             <button onClick={() => navigate(-1)} className="mt-4 px-4 py-2 bg-brand-yellow rounded-md text-dark-bg">Go Back</button>
          </div>
      )
  }
  ...
  return (
    <div className="flex flex-col min-h-screen max-w-md mx-auto bg-[#F8F9FA] relative animate-in slide-in-from-right">
      ...
      <div className="p-4 flex-1 overflow-y-auto space-y-4 pb-24">
        <div>
           <h3 className="text-[20px] font-bold text-[#0F172A]">Order Summary</h3>
               ...
               <div key={idx} className="flex w-full justify-between py-2">
                   <div className="flex items-center">
                       <div className="w-6 h-6 bg-[#E2E8F0] rounded-[4px] flex items-center justify-center">
                           <span className="text-[12px] font-bold text-[#0F172A]">{item.quantity}x</span>
                       </div>
                       <span className="ml-3 text-[16px] text-[#0F172A]">{item.name}</span>
                   </div>
                   <span className="text-[16px] font-medium text-[#0F172A]">₹{item.price * item.quantity}</span>
               </div>
           ))}
        </div>
        ...
        <div className="bg-white rounded-[12px] shadow-[0_2px_4px_rgba(0,0,0,0.05)] w-full p-4">
           <div className="flex w-full justify-between py-1">
               ...
               <span className="text-[#0F172A] text-[14px]">₹{subtotal.toFixed(2)}</span>
           </div>
           <div className="flex w-full justify-between py-1">
               <span className="text-gray-500 text-[14px]">Taxes</span>
               <span className="text-[#0F172A] text-[14px]">₹{tax.toFixed(2)}</span>
           </div>
           <div className="flex w-full justify-between py-1">
               <span className="text-gray-500 text-[14px]">Delivery Fee</span>
               <span className="text-[#0F172A] text-[14px]">{order.deliveryFee > 0 ? \`₹\${order.deliveryFee.toFixed(2)}\` : 'TBD'}</span>
           </div>
           
           <hr className="my-3 border-[#F1F5F9]" />
           
           <div className="flex w-full justify-between items-center">
               <span className="font-bold text-[#0F172A] text-[18px]">Grand Total</span>
               <span className="font-bold text-[#0F172A] text-[18px]">₹{grandTotal.toFixed(2)}</span>
           </div>
        </div>
      </div>

      {order.status === "Pending Approval" && (
         <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white shadow-[0_-4px_6px_rgba(0,0,0,0.05)] p-4">
            <button 
...
fs.writeFileSync('src/components/DigitalBillScreen.tsx', content);
