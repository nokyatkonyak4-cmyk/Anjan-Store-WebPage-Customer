...
export function OrderHistoryScreen({ orders, onNavigate }: any) {
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col h-full bg-gray-50">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    ...
                    {reviewOrder.items && reviewOrder.items.map((item: any, index: number) => {
                        ...
                        const reviewData = itemReviews[product.id] || { rating: 0, feedback: "" };
                        ...
                        return (
                            <div key={index} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 space-y-4">
                                <div className="flex items-start space-x-4">
                                    <img src={product.imageUrls?.[0] || product.imageUrl || "/AppIcon-512x512.png"} className="w-16 h-16 object-contain rounded-md border border-gray-100 p-1 bg-gray-50" />
                                    <div className="flex-1">
                                        <h4 className="font-bold text-sm text-dark-bg line-clamp-2">{product.name}</h4>
                                        <div className="flex space-x-1 mt-2">
                                            {[1, 2, 3, 4, 5].map((star) => (
                                                <button key={star} onClick={() => handleItemRatingChange(product.id, star)} className="p-1 transition-transform active:scale-90">
                                ...
                                {hasRated && (
                                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mt-4 pt-4 border-t border-gray-50">
                                        <textarea
                                            ...
                                            onChange={(e) => handleItemFeedbackChange(product.id, e.target.value)}
...
`;
fs.writeFileSync('src/components/OrderHistoryScreen.tsx', content);
console.log('Done rewriting OrderHistoryScreen');
