...
content = content.replace(/export function FavoritesScreen\(\{[\s\S]*?\}\s*\)\s*;\s*\}/, newFavoritesScreen);
...
content = content.replace(/export function CategoryScreen\(\{[\s\S]*?\}\s*\)\s*;\s*\}/, newCategoryScreen);

fs.writeFileSync('src/components/Screens.tsx', content);
console.log("Updated FavoritesScreen and CategoryScreen");
